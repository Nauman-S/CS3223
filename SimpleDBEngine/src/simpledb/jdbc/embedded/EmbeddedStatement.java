...
class EmbeddedStatement extends StatementAdapter {
	...
	/**
	 * Executes the specified SQL query string. Calls the query planner to create a
	 * plan for the query, and sends the plan to the ResultSet constructor for
	 ...
	 * update planner and then committing. Rolls back and throws an SQLException on
	 * an error.
	 */
	...
	/**
	 * Executes the specified SQL query string. Calls the query planner to create a
	 * plan for the query. Rolls back and throws an SQLException if it cannot create
	 * the plan.
	 */
	public Plan getQueryPlan(String sql) throws SQLException {
		try {
			Transaction tx = conn.getTransaction();
			return planner.createQueryPlan(sql, tx);
		} catch (RuntimeException e) {
			conn.rollback();
			throw new SQLException(e);
		}
