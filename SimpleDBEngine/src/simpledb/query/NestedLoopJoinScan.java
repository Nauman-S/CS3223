...
public class NestedLoopJoinScan implements Scan {
    ...
    /**
     * Creates a nested loop join scan for the specified LHS scan and RHS scan.
     *
     * @param lhs       the LHS scan
     * @param rhs       the RHS scan
     * @param joinfield the LHS field used for joining
     * @param fieldRhs  the field rhs
     */
    ...
    public int getInt(String fldname) {
        if (rhs.hasField(fldname)) return rhs.getInt(fldname);
        else return lhs.getInt(fldname);
    }

    @Override
    public String getString(String fldname) {
        if (rhs.hasField(fldname)) return rhs.getString(fldname);
        else return lhs.getString(fldname);
    }

    @Override
    public Constant getVal(String fldname) {
        if (rhs.hasField(fldname)) return rhs.getVal(fldname);
        else return lhs.getVal(fldname);
    }
    ...
    /**
     * Returns true if there is a next iteration.
     *
     * @return the boolean
     */
